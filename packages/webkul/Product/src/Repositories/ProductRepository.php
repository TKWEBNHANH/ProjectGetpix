...
class ProductRepository extends Repository
{
    ...
    public function isExistProDuctByCategoryId($categoryId){
        $products = \DB::table('product_categories')->where('category_id', '=', $categoryId)
            ->count();
        if ( $products > 0 ){
            return true;
        } else{
    ...
    public function getProductSameAccount($productId){
        $product = DB::table('product_users')
                    ->select('*')
                    ->join('product_users as pu', 'product_users.admin_id', '=', 'pu.admin_id')
                    ->join('product_attribute_values', 'product_attribute_values.product_id', '=', 'product_users.product_id')
                    ->join('product_images', 'product_images.product_id', '=', 'product_users.product_id')
     ...
     public function getSkuByProductId($productId){
        $product = DB::table('product_attribute_values')
                    ...
                    ->where('product_attribute_values.attribute_id', 3)
                    ->get();
         return $product;
    }

    public function getTotalProductAndAccountInfo($adminId){
       $user_info = DB::table('product_users AS pu')
                 ->select('admins.name', DB::raw('COUNT(`pu`.`product_id`) as total'))
                 ->join('admins', 'pu.admin_id', '=', 'admins.id')
